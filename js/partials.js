...
Handlebars.registerPartial(
    ...
    <nav id="notes-interaction" role="navigation">
        <div class="button-group" role="menu">
            <button type="button" onclick="Notes_Main.sortByDueDate()" role="button">By due date</button>
            <button type="button" onclick="Notes_Main.sortByCreationDate()" role="button">By creation date</button>
            <button type="button" onclick="Notes_Main.sortByImportance()" role="button">By importance</button>
        </div>
        <div class="button-group" role="menu">
            <button type="button" onclick="Notes_Main.filterFinishedNotes()" role="button">Show finished notes</button>
        </div>
    </nav>
    ...
    `<form action="javascript:Notes_Main.saveNote()" method="POST" role="form">
        ...
        <div class="form-group" role="group">
            ...
            <div id="note-importance" role="radiogroup">
                <input id="very-low-importance" name="note-importance" type="radio" value="1" {{#if (ifExpr importance '===' '1')}}checked="checked" aria-checked="true" {{else}} aria-checked="false" {{/if}} role="radio" /><label for="very-low-importance"><i class="fas fa-bolt fa-fw" aria-label="Very low importance"></i></label>
                <input id="low-importance" name="note-importance" type="radio" value="2" {{#if (ifExpr importance '===' '2')}}checked="checked" aria-checked="true" {{else}} aria-checked="false" {{/if}} role="radio" /><label for="low-importance"><i class="fas fa-bolt fa-fw" aria-label="Low importance"></i></label>
                <input id="normal-importance" name="note-importance" type="radio" value="3" {{#if (ifExpr importance '===' '3')}}checked="checked" aria-checked="true" {{else}} aria-checked="false" {{/if}} role="radio" /><label for="normal-importance"><i class="fas fa-bolt fa-fw" aria-label="Normal importance"></i></label>
                <input id="high-importance" name="note-importance" type="radio" value="4" {{#if (ifExpr importance '===' '4')}}checked="checked" aria-checked="true" {{else}} aria-checked="false" {{/if}} role="radio" /><label for="high-importance"><i class="fas fa-bolt fa-fw not-set" aria-label="High importance"></i></label>
                <input id="very-high-importance" name="note-importance" type="radio" value="5" {{#if (ifExpr importance '===' '5')}}checked="checked" aria-checked="true" {{else}} aria-checked="false" {{/if}} role="radio" /><label for="very-high-importance"><i class="fas fa-bolt fa-fw not-set" aria-label="Very high importance"></i></label>
            </div>
